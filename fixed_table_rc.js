...
Copyright (C) (2014) Sander Baas (minor modifications)
...
(function ($) {
	...
	$.fn.fxdHdrCol = function (o) {
		var cfg = {
			...
			width: 0,
			fixedCols: 0,
			colDefault: { width: 50, align: 'left' },
			colModal: [],
			tableTmpl: function () {
				return '<table />';
			},
		...
		return this.each (function () {
			...
			$ths.each(function (i, el) {
				...
				for (var j = 0; j < el.colSpan; j++) {
					if (cfg.colModal.length > 0) {
						...
						calcWidth += cfg.colDefault.width;
					}
					ct++;
				}
				...
				if (cfg.colModal.length > 0) {
					...
					$(el).css({width: calcWidth, textAlign: cfg.colDefault.align});
				}
			...
			$('tbody', $this).find('tr').each(function (i, el) {
				$('td', el).each(function (i, tdel) {
					if (cfg && cfg.colModal && cfg.colModal[i] && cfg.colModal[i].align) {
